...
遇到登录、验证码、二维码扫描或其它自动化无法完成的交互时，调用此方法弹出浏览器窗口，让用户手动完成操作后再继续。
...
调用后应**立即停止当前对话**，告知用户接管浏览器，等待用户完成操作并发起新的提问后再继续。用户重新发起对话后，应**先调用 `browser.get_interactive_elements()` 读取当前页面快照**，了解浏览器现状后再决策下一步操作。

> **禁止截图二维码**：遇到二维码登录时，**不允许**先截图再把二维码图片展示给用户扫码。二维码存在有效期，截图传输过程中极易过期导致扫码失败。应直接调用 `request_manual()` 弹出浏览器窗口，让用户在本机实时扫码。
...
| 需要登录、验证码或手动步骤 | 调用 `browser.request_manual()` 弹出浏览器，等待用户手动完成后发起新提问 |
| 遇到二维码登录 | **禁止截图二维码**，直接调用 `browser.request_manual()` 让用户实时扫码（截图可能过期） |
| 报错 `[Errno 61] Connect call failed` 或 `Failed to connect to browser` | 浏览器进程已关闭，调用 `browser.navigate(url)` 即可——该方法会自动重新启动浏览器并打开页面 |
