...
- 禁止使用 `add_textbox` / `add_shape` / `add_picture` 等 python-pptx 原生 API 新增元素 — **必须用 `copy_shape`**
- 禁止直接操作 python-pptx 底层 API 修改文字（`font.color.rgb = ...`、`run.font.size = ...`、`tf.paragraphs` 等） — **必须用 `edit_text`**
- 禁止凭空指定坐标 — **必须先 `query_slide` 获取现有布局再推算**
- `copy_shape` 已完整复制源元素的样式，添加元素时无需通过 XML 查看源 shape 的样式信息
...
| 查询页面信息 | `query_slide` |
| 新增元素 | `copy_shape` |
| 移动/调整尺寸 | `move_shape` / `resize_shape` |
| 删除元素 | `delete_shape` |
| 页面增删/复制/排序 | `slide_organize` |
| 图片导出/替换 | `export_image` / `replace_shape_image` |

### 默认行为

- **保留原始样式**：编辑时保持模板的字体、配色、布局风格，不做无根据的风格变更。
- **单位**：所有布局参数均使用**厘米（cm）**。python-pptx 内部使用 EMU，可通过 `emu_to_cm()` / `cm_to_emu()` 转换。
...
- **保存时机**：每次编辑操作（增删改页面或元素）完成后，立即 `prs.save(path)` 回原文件。纯查询操作不保存。每次执行编辑前，先 `Presentation(path)` 重新加载文件，避免覆盖用户在另一窗口所做的修改。
- **新增页面位置**：用户说"在第 i 页新增一页"时，新页面插入到第 i 页之前（即新页面成为新的第 i 页，原第 i 页及之后顺延）。

### `edit_text` 的内存修改语义（重要）

`edit_text` 修改的是传入的 `prs` 对象的**内存状态**，不会自动持久化到磁盘。其行为要点：

1. **修改后可直接查询**：调用 `edit_text` 后，无需重新 `Presentation(path)` 加载，直接用 `query_shape(slide, shape_id, ...)` 即可查到修改后的结果。
2. **重新加载会丢失修改**：如果在两次 `edit_text` 之间执行了 `prs = Presentation(path)`，则前一次 `edit_text` 的修改会全部丢失。
3. **正确的工作流**：一次性加载 `prs` → 执行所有 `edit_text` / `copy_shape` / `move_shape` 操作 → 调用一次 `prs.save(path)` 持久化。
...
edit_text(prs, 0, shape_id=5, operations=[
    {"action": "set_run_text", "para_index": 0, "run_index": 0, "text": "新标题"},
])

edit_text(prs, 0, shape_id=8, operations=[
    {"action": "add_para", "text": "新段落"},
])
prs.save("output.pptx")

# ❌ 错误：每次 edit_text 后重新加载，导致前一次修改丢失
prs = Presentation("output.pptx")
edit_text(prs, 0, shape_id=5, operations=[...])

prs = Presentation("output.pptx")  # ← 这会丢弃上一步的修改！
edit_text(prs, 0, shape_id=8, operations=[...])
```
...
添加或移动元素时，**必须基于 `query_slide` 获取的实际布局来推算坐标**，禁止凭空指定。布局推算遵循以下原则：

1. **先算后动**：在修改任何元素之前，先基于 `query_slide` 结果完成空间计算（可用高度、可用宽度、已有元素占用情况），确认方案可行再执行，避免先动手再反复检测修正。文本框的布局信息中已自动包含 `text_render` 字段（文本实际渲染宽高及是否溢出），直接据此推算所需空间即可，禁止凭直觉分配高度导致文字截断或溢出。空间计算时，必须使用 `query_slide` 返回的 `text_render`（文本实际渲染高度）作为内容占用空间的唯一依据。如果修改会改变已有元素的尺寸，应先验证 `text_render` 确认内容不会溢出。
2. **尺寸由空间和内容决定**：新元素的尺寸应根据可用空间和内容量独立计算，不要从被复制的元素继承尺寸。`copy_shape` 只复用视觉样式（填充、边框等），尺寸参数应在复制后重新设定。
...
- 如果用户指定的空间不足以容纳新元素，应先通过 `query_slide` 计算可用空间，然后向用户说明情况并提出至少两种替代方案供选择，**禁止在未告知用户的情况下缩小或移动已有元素以腾出空间**

### 页面尺寸参考

标准 16:9 宽屏页面约 33.87 x 19.05 cm，计算时以此为准（可通过 `prs.slide_width` / `prs.slide_height` 精确获取）。
...
import sys
sys.path.insert(0, "{skill_path}/pptx/scripts")
...
from edit_operations import (
    slide_organize, get_shape, copy_shape, delete_shape,
    move_shape, resize_shape, edit_text, set_run_font, get_available_fonts,
    export_image, replace_shape_image,
)
```
...
### edit_text(prs, slide_index, shape_id, operations)
...
edit_text(prs, 0, shape_id=5, operations=[
    # 修改文字和格式
    {"action": "set_run", "para_index": 0, "run_index": 0,
     "text": "新标题",
     "format": {"bold": True, "color_rgb": "FF0000", "size_pt": 36}},
    # 修改段落对齐
    {"action": "set_para_format", "para_index": 1,
     "format": {"alignment": "center", "line_spacing_pt": 24}},
    # 追加新 run
    {"action": "add_run", "para_index": 0,
     "text": "追加的文字", "format": {"size_pt": 14}},
    # 追加新段落
...
### move_shape(slide, shape_id, left_cm=None, top_cm=None, width_cm=None, height_cm=None)

移动元素或同时调整尺寸。参数为绝对坐标（cm），Group 内元素自动处理逆变换。未指定的参数保持原值。

```python
move_shape(slide, shape_id=3, left_cm=5.0, top_cm=10.0)
move_shape(slide, shape_id=3, left_cm=5.0, top_cm=10.0, width_cm=8.0, height_cm=4.0)
```

### resize_shape(slide, shape_id, width_cm=None, height_cm=None)

仅调整尺寸，不改变位置。

```python
resize_shape(slide, shape_id=3, width_cm=10.0, height_cm=5.0)
```

### copy_shape(slide, src_shape_id, left_cm=None, top_cm=None, width_cm=None, height_cm=None, new_name=None, z_order=None)

复制元素到幻灯片顶层。只复制 `src_shape_id` 指向的**单个元素**，不连带其所在 Group。未指定的位置/尺寸参数使用源元素的绝对坐标。

返回新 shape 的 shape_id。

```python
new_id = copy_shape(slide, src_shape_id=5, left_cm=10.0, top_cm=8.0)
new_id = copy_shape(slide, src_shape_id=3)  # 原位置复制
```

### delete_shape(prs, slide_index, shape_id)

删除元素。支持删除 Group 内子 shape（仅移除该子 shape，不影响其余）。

```python
delete_shape(prs, 0, shape_id=5)
```

### get_shape(slide, shape_id)

按 shape_id 获取 shape 对象。支持递归查找 Group 内部。获取到 shape 对象后可直接操作 python-pptx 原生属性。

### export_image(slide, shape_id, output_path)
...
| `shape_id` | 目标 shape 的数字 id |
| `output_path` | 导出路径（扩展名决定格式，如 `.png` / `.jpg`） |
...
```python
{
    "success": True,
    "shape_id": 3,
    "image_type": "fill",       # "picture" 或 "fill"
...
```python
export_image(slide, shape_id=2, output_path="logo.png")
export_image(slide, shape_id=3, output_path="background.jpg")
```

### replace_shape_image(slide, shape_id, image_path)

替换 shape 中的图片。支持独立图片（`<p:pic>`）和 fill 填充图片（`fill_type=6`）。创建新的 image part，将 XML 中 `<a:blip r:embed="...">` 指向新 part。
...
```python
replace_shape_image(slide, shape_id=2, image_path="new_logo.png")
replace_shape_image(slide, shape_id=3, image_path="new_bg.jpg")
```
...
### get_available_fonts(category=None, keyword=None)

查询系统可用字体。category 支持 `"chinese"` / `"latin"` / `None`（全部）。
...
### set_run_font(run, latin, east_asia)

在 run 级别分别设置西文和中文字体。python-pptx 的 `font.name` 只能修改 `<a:latin>`，本函数通过 XML 操作同时设置 `<a:ea>`。

```python
set_run_font(run, latin="Arial", east_asia="微软雅黑")
```
...
| 操作 | 函数 | 说明 |
|------|------|------|
| 导出图片 | `export_image(slide, shape_id, output_path)` | 将图片保存为独立文件 |
| 替换图片 | `replace_shape_image(slide, shape_id, image_path)` | 用新图片替换原图 |
| 复制图片 | `copy_shape(slide, src_shape_id, ...)` | 两种图片形式均支持（同页复制，rId 天然兼容） |
| 删除图片 | `delete_shape(prs, slide_index, shape_id)` | 删除整个 shape（含图片） |

> **注意**：fill 图片的显示效果受填充模式（stretch、tile、clip）影响。`query_shape` 的 `style` 返回的是图片文件信息，不包含填充模式。如需修改填充模式，需直接操作 XML。
...
- **color_rgb 格式**：`edit_text` 的 `color_rgb` 接受 `"FF0000"` 或 `"#FF0000"` 格式，不带 `#` 前缀也可以。
- **font.name 的局限**：`run.font.name = "微软雅黑"` 只设置西文字体 `<a:latin>`，中文字体需用 `set_run_font(run, east_asia="微软雅黑")` 或 `edit_text` 的 `"east_asia"` 字段。
- **段落索引与视觉行不一致**：python-pptx 的 paragraph 按硬回车分段，一个文本框内可能只有 1 个 paragraph 但视觉上多行显示。
- **z_order**：python-pptx 没有直接修改 z_order 的 API，可通过调整 XML 中 shape 元素的顺序间接实现。
- **`RGBColor.from_string` 不接受带 `#` 的字符串**：传参时去掉 `#` 前缀，正确写法为 `RGBColor.from_string("2C4A6E")` 而非 `RGBColor.from_string("#2C4A6E")`。
- **绕过 edit_text 直接操作底层 API 导致报错**：这是最高频的错误来源。所有文字修改必须通过 `edit_text` 的 action 完成。图表等特殊功能可按对应参考文档使用 python-pptx API。
- **edit_text 后重新加载 prs 导致修改丢失**：`edit_text` 修改的是内存中的 `prs` 对象，不会自动持久化。不要在两次编辑操作之间重新 `Presentation(path)` 加载文件，否则前一次的修改会全部丢失。详见上方"edit_text 的内存修改语义"章节。
