...
function _strLitXml(categories) {
    let xml = `<c:strLit xmlns:c="${C}"><c:ptCount val="${categories.length}"/>`;
    for (let i = 0; i < categories.length; i++) {
        xml += `<c:pt idx="${i}"><c:v>${_escXml(String(categories[i]))}</c:v></c:pt>`;
    }
    xml += '</c:strLit>';
    return xml;
}
...
function _numLitXml(values) {
    let xml = `<c:numLit xmlns:c="${C}"><c:formatCode>General</c:formatCode><c:ptCount val="${values.length}"/>`;
    for (let i = 0; i < values.length; i++) {
        xml += `<c:pt idx="${i}"><c:v>${values[i]}</c:v></c:pt>`;
    }
    xml += '</c:numLit>';
    return xml;
}
...
function generateBarChartXml(opts) {
    ...
    for (let i = 0; i < series.length; i++) {
        ...
        seriesXml += `<c:ser><c:idx val="${i}"/><c:order val="${i}"/>` +
            `<c:tx><c:v>${_escXml(s.name)}</c:v></c:tx>` +
            `<c:spPr><a:solidFill><a:srgbClr val="${color}"/></a:solidFill></c:spPr>` +
            `<c:cat>${_strLitXml(s.categories)}</c:cat>` +
            `<c:val>${_numLitXml(s.values)}</c:val>` +
            `</c:ser>`;
    }
