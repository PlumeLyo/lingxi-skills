...
流程分三步：根据内容基调设计视觉风格 → 用 `write_file` 编写语义化 HTML → 通过 `python_cell_exec`工具 执行转换脚本输出 .docx。
...
### 第 2 步：用 write_file 编写 HTML 文件

> **强制规则：必须使用 `write_file` 写入 HTML 文件。禁止通过 Python 代码生成 HTML 内容。**

1. 直接以 `<!DOCTYPE html>` 开头输出完整 HTML（禁止使用 ` ```html ` 等 markdown 代码围栏包裹，直接输出原始 HTML 内容）
...
> **`python_cell_exec`工具 仅用于调用 html2docx 转换脚本**，不要用它来生成或写入 HTML 内容。

通过 **`python_cell_exec`工具** 执行以下代码：
...
通过 `edit_docx()` 完成，内部流程：解包 → 合并相邻同样式 run → 文本替换 → 重打包 → 校验。通过 **`python_cell_exec`工具** 执行：
...
# 用 python_cell_exec工具 读取并修改 unpacked/word/*.xml
