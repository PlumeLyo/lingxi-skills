---
name: tag-organize
description: 笔记标签整理的核心原则与完整工作流程，支持 MCP 和 CLI 两种模式。当用户提到"整理笔记标签"、"清理标签"、"标签太乱"、"标签太多"、"帮我打标签"、"重构标签"、"重新分类"、"笔记分类混乱"、"标签体系需要优化"等需求时使用。
...
本 Skill 支持两种操作模式：
- **MCP 模式**：通过 WPS 笔记 MCP 工具操作（默认）
- **CLI 模式**：通过 wpsnote-cli 命令行操作（MCP 不可用时自动降级）
