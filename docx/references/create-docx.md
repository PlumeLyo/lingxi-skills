...
编写 JS 脚本，通过 `run_node_docx()` 执行生成 `.docx`。
...
**写入方式**：使用 `write_file(path, content, mode="write")` 一次性写入完整 JS 代码。长篇文档（正文超过 2000 字）必须用 `mode="append"` 分章写入，详见下方「长篇创作」。

**编码注意**：中文文本直接写原始字符，不要写 `\uXXXX` 转义。避免 emoji 和不常见 Unicode 符号。

**空文件陷阱**：`write_file` 的 `content` 参数不能为空字符串，否则文件将为空，执行必然失败。
...
**核心原则：所有 append 都是往同一个 JS 文件追加代码，不是创建独立模块。** `h`、`refs`、`COLORS` 等变量在骨架中定义一次，后续 append 的代码处于同一文件作用域，可直接使用这些变量，**禁止在 append 中重复 require 或重新定义 `h`**。

1. 第一次 `write_file(path, content, mode="write")`：写入 require、`h` 初始化、`refs`、常量、工具函数、封面、目录等**骨架代码**，不写 `h.build()` 和文件末尾
2. 每章用 `write_file(path, content, mode="append")`：续写该章节的函数定义，函数内直接使用骨架中已有的 `h`、`refs` 等变量
...
通过 **`python_cell_exec`** 执行，`run_node_docx()` 内部自动修复常见问题并预检，执行成功即生成完成：
...
output = r'<OUTPUT_ROOT>/xxx.docx'
run_node_docx(r'<OUTPUT_ROOT>/xxxx.js', output=output)
```

## 错误恢复

**定点修复，禁止从零重写，修完后回到 Phase 3 重新执行**

1. 报错信息已含行号和上下文 → 读取出错文件确认位置，**只修改出错的那个函数**
2. 如果改坏了更多地方 → 用 `restore_backup()` 恢复后再做定点修改：

```python
from run_code_docx import restore_backup
restore_backup(r'<OUTPUT_ROOT>/xxxx.js')
```
...
- 禁止在 `write_file` 写入 JS 文件的同一轮中调用 `python_cell_exec`——先写完文件，再执行
...
| 问题 | 处理方式 |
|------|----------|
| JS 文件为空（0 字节） | `write_file` 的 `content` 为空。删除空文件，回到 Phase 2 重新写入。**禁止改用 Python `open()` 写入** |
| `FileNotFoundError` | 检查 `run_node_docx(output=...)` 的路径是否正确 |
